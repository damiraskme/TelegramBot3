...
def downloadVideo(download_link, id):
    ...
    data = {
        'id': 'https://vm.tiktok.com/ZMjYw7ePS/',
        'locale': 'en',
        'tt': 'Y0h6Q1Bk',
    }
    ...
    with open(f"messages/tiktok_videos/file_{id}.mp4", "wb") as output:
        while True:
...
def downloadTiktok(video: str, id: str):
    ...
    with open(f"tiktok_videos/file_{id}.mp4", "wb") as file:
...
def deleteTiktok(id: str):
    try:
        if (os.path.isfile(f"messages/tiktok_videos/file_{id}.mp4")):
            os.remove(f"messages/tiktok_videos/file_{id}.mp4")
